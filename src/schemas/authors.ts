...
export const authorsSchema = ({ image }: SchemaContext) => zod.object({
	...
	socialMedia: zod.record(zod.string(), zod.string().url()).optional().describe('The author social media links, if they want to share. Formatted as an object where the key is the name and the value the url.')
});
